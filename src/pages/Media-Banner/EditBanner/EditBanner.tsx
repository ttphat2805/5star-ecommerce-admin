...
import { BannerType, ResponseType } from '~/utils/Types';
...
const EditBanner = () => {
    ...
    const getBanners = () => {
        setLoading(true);
        MediaService.getBanner(Number(id)).then((res: ResponseType) => {
            if (res.statusCode === 200) {
                setLoading(false);
                const imagePatch = `${Config.apiUrl}upload/${res.data.media.file_name}`;
                patchFormValue(res.data);
                setImageURL(imagePatch);
            } else {
                setLoading(false);
            }
        });
    ...
    const handleSubmitForm = async (values: BannerType) => {
        ...
        // }
        setLoading(true);
        let imageResUpload: number | any;
        if (fileBanner) {
            imageResUpload = await UploadService.requestUploadImage(fileBanner[0]);
        }
        ...
        MediaService.updateBanner(dataSendRequest, Number(id)).then(
            (res: ResponseType) => {
